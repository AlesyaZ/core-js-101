...
function getDigitalRoot(num) {
  ...
  const number = str.reduce((a, b) => Number(a) + Number(b));
  const sumNum = String(number).split('');
  return sumNum.reduce((a, b) => Number(a) + Number(b));
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < bracketsConfig.length; i += 1) {
    while (resultStr) {
      if (resultStr.includes(bracketsConfig[i])) {
        resultStr = resultStr.replace(bracketsConfig[i], '');
        i = 0;
      } else break;
    }
  }
